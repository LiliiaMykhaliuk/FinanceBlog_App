...
def test_total_values_appear_on_list_page(user_transactions, client):
    ...
    income_total = sum(t.amount_in_usd for t in user_transactions if t.type == 'income')
    expense_total = sum(t.amount_in_usd for t in user_transactions if t.type == 'expense')
...
def test_add_transaction_request(user, transaction_dict_params, client):
    ...
    assertTemplateUsed(response, 'app/partials/transaction-success.html')
...
def test_cannot_add_transaction_with_negative_amount(
    ...
    assertTemplateUsed(response, 'app/partials/create-transaction.html')
...
def test_update_transaction_request(user, transaction_dict_params, client):
    ...
    transaction_dict_params['currency'] = 'SEK'
    ...
    assert Transaction.objects.filter(user=user).count() == 1
    transaction = Transaction.objects.first()
    assert transaction.amount_in_usd == convert_to_EUR(40, 'SEK')
...
def test_delete_transaction_request(user, transaction_dict_params, client):
    ...
    client.delete(
        reverse('delete-transaction', kwargs={'pk': Transaction.objects.first().pk})
    )

    # check the request has DELETED the transaction
